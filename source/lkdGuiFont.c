...
/**
  ...
  *@retval None
  */
void GuiText(fontTextInfo *tInfo, char *str)
{
...
/**
  ...
  *@retval 像素长度
  */
uint16_t GetTextPixLen(char *str)
{
	uint16_t len = 0;
	while(*str != '\0'){
		if(*str > 0x7f){
...
/**
  ...
  *@retval 像素长度
  */
void GuiRowText(lkdCoord x, lkdCoord y,uint16_t wide, FontFlag flag, char *str)
{
	uint16_t pixLen;
	fontTextInfo tInfo;
	lkdColour backcolor; 
...
void GuiFont12Align(lkdCoord x, lkdCoord y,uint16_t wide, FontFlag flag, char *str)
{
	GuiRowText(x, y,wide,flag,str);
}

void GuiFont12(lkdCoord x, lkdCoord y, char *str)
{
